...
export function formatLevelLabel(t: string, n: string): string {
  return labelLevel(t, n);
}

/** @deprecated Use detectOverrides instead */
export function detectHierarchyOverrides(
  childLevels: HierarchyLevel[],
  parentLevels: HierarchyLevel[],
  ...
  for (const parent of parentLevels)
    for (const child of childLevels)
      for (const note of detectOverrides(
        parent as unknown as object,
        child as unknown as object,
      ))
        results.push({ childDecision: "", parentConstraint: "", note });
  return results;
}
...
export function computeExtensionInheritance(
  ...
  for (const [idx, lvl] of levels.entries()) {
    inherited = [
      ...new Set([...inherited, ...extractLevel(lvl, idx).extensions]),
    ];
  }
  return inherited;
}
...
export function computeOntologyInheritance(
  ...
  let inherited: OntologyMetadataEntry[] = [];
  for (const [idx, lvl] of levels.entries()) {
    const n = extractLevel(lvl, idx);
    const excludeNames = new Set(n.ontologies.flatMap((o) => o.excludes ?? []));
    const filtered = inherited.filter((o) => !excludeNames.has(o.name));
    const ownNames = new Set(n.ontologies.map((o) => o.name));
    inherited = [
      ...filtered.filter((o) => !ownNames.has(o.name)),
      ...n.ontologies,
    ];
  }
  return inherited;
